document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fallback data in case of CORS restriction when opening index2.html locally via file:// protocol
    const fallbackData = [
        {
            "title": "Style Scalpel",
    ...
    function renderContent(data) {
        ...
        data.forEach((app, index) => {
            ...
            // 2. Populate Static Graph Node (SVG) Content
            const nodeLink = document.querySelector(`.graph-node-${index + 1}`);
            if (nodeLink) {
                let areaText = app.area || "Workshop Tool";
                if (areaText.length > 18) {
                    areaText = areaText.split(' ')[0]; // Wrap long names
                }
                nodeLink.innerHTML = `
                    <span class="node-index">${indexStr}</span>
                    <span class="node-area">${areaText}</span>
                    <span class="node-title">${app.title}</span>
                `;
            }
